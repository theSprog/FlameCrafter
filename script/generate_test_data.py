...
class FlameTestDataGenerator:
    def __init__(self):
        # 模拟真实的函数调用栈
        self.system_funcs = ["main", "__libc_start_main", "_start", "exit"]
    ...
    def generate_test_data(
        self, output_file: str, sample_count: int = 1000, realistic: bool = True, hot_ratio: float = 0.3
    ):
        """
        生成测试数据
...
def main():
    ...
    parser.add_argument("-o", "--output", default="test_data.txt", help="Output file name (default: test_data.txt)")
    parser.add_argument("-n", "--samples", type=int, default=1000, help="Number of samples to generate (default: 1000)")
    parser.add_argument("--simple", action="store_true", help="Generate simple test data instead of realistic")
    parser.add_argument("--hot-ratio", type=float, default=0.3, help="Ratio of hot path samples (default: 0.3)")

    # 预定义的配置
    parser.add_argument("--small", action="store_true", help="Generate small dataset (1K samples)")
    parser.add_argument("--medium", action="store_true", help="Generate medium dataset (10K samples)")
    parser.add_argument("--large", action="store_true", help="Generate large dataset (100K samples)")
    parser.add_argument("--huge", action="store_true", help="Generate huge dataset (1M samples)")
    ...
    try:
        generator.generate_test_data(
            output_file=args.output, sample_count=args.samples, realistic=not args.simple, hot_ratio=args.hot_ratio
        )
